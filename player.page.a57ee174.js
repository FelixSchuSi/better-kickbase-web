import{r as b,e as c,s as y,$ as p,y as g,n as v,a as R,b as u,i as h,t as Z}from"./client-shim.158a9d44.js";function k(e){switch(e){case 1:return"TW";case 2:return"ABW";case 3:return"MIT";case 4:return"ST";case 9999999999:default:return""}}var I=Object.defineProperty,M=Object.getOwnPropertyDescriptor,d=(e,t,i,s)=>{for(var a=s>1?void 0:s?M(t,i):t,r=e.length-1,l;r>=0;r--)(l=e[r])&&(a=(s?l(t,i,a):l(a))||a);return s&&a&&I(t,i,a),a};const x="bkb-player-badges";let n=class extends y{render(){return p`
      <div class="badges-container">
        ${this.singleBadge(String(this.number))}
        ${this.singleBadge(k(this.position))}
        ${this.singleBadge(this.status)}
      </div>
    `}singleBadge(e){return e===void 0?"":p`
      ${g`
        <defs>
          <mask id="mask-${e}" x="0" y="0" >
            <rect x="0" y="0" width="50" height="25" fill="#fff" />
            <text text-anchor="middle" x="25" y="18" dy="1">${e}</text>
          </mask>
        </defs>
        <rect x="0" y="0" width="50" height="25" mask="url(#mask-${e})" fill-opacity="1" />
    `}
    `}};n.styles=b`
    .badges-container {
      display: flex;
    }

    svg {
      transform: skew(-10deg);
      height: 25px;
      width: 54px;
    }
  `;d([c({type:Number})],n.prototype,"number",2);d([c({type:String})],n.prototype,"position",2);d([c({type:String})],n.prototype,"status",2);n=d([v(x)],n);var U="/better-kickbase-web/assets/2.b9770161.png",Y="/better-kickbase-web/assets/3.c4ecc237.png",S="/better-kickbase-web/assets/4.18b4e12c.png",E="data:image/png;base64,",P="/better-kickbase-web/assets/7.e4732b84.png",B="/better-kickbase-web/assets/9.0dfad2eb.png",N="/better-kickbase-web/assets/11.0b614101.png",H="/better-kickbase-web/assets/13.9d841356.png",G="/better-kickbase-web/assets/14.63efdbd8.png",O="data:image/png;base64,",T="/better-kickbase-web/assets/18.3780ff0a.png",Q="/better-kickbase-web/assets/19.0896da6a.png",W="/better-kickbase-web/assets/20.ea3c24b0.png",j="data:image/png;base64,",F="data:image/png;base64,",J="/better-kickbase-web/assets/28.a4cea9d1.png",L="data:image/png;base64,",D="/better-kickbase-web/assets/43.a88206a1.png",z=Object.defineProperty,C=Object.getOwnPropertyDescriptor,w=(e,t,i,s)=>{for(var a=s>1?void 0:s?C(t,i):t,r=e.length-1,l;r>=0;r--)(l=e[r])&&(a=(s?l(t,i,a):l(a))||a);return s&&a&&z(t,i,a),a};const f={team_2:U,team_3:Y,team_4:S,team_5:E,team_7:P,team_9:B,team_11:N,team_13:H,team_14:G,team_15:O,team_18:T,team_19:Q,team_20:W,team_22:j,team_24:F,team_28:J,team_40:L,team_43:D},X="bkb-player-points";let m=class extends y{constructor(){super(...arguments);this.maxPoints=0,this.rootRef=R()}willUpdate(e){e.has("points")&&!!this.points?.seasons&&(this.maxPoints=Math.max(...this.points.seasons.flatMap(t=>t.matches).map(t=>t.points)))}render(){return p`
      <div class="root" ${u(this.rootRef)}>
        ${this.points.seasons.map(e=>this.seasonTemplate(e,e.year))}
        ${this.upcomingMatches.map(e=>this.matchTemplate(e))}
      </div>
    `}firstUpdated(e){this.rootRef.value?.scrollTo({left:99999999})}seasonTemplate(e,t){return p`
      <div class="season">
        ${e.matches.map(i=>this.matchTemplate(i,t))}
        <div class="season-summary">
          ${e.year}</br>
          <small>Saison Auswertung</small>
          ${e.points}</br>
          <small>Punkte</small>
          ${Math.round(e.points/e.appearances)}</br>
          <small>∅ Punkte</small>
          ${e.appearances}</br>
          <small>Einsätze</small>
          ${e.startingEleven}</br>
          <small>Startelf</small>
        </div>
      </div>
    `}matchTemplate(e,t){const i=e.points>=100?"#25c28b":"#ea5f42",s=`${Math.round(e.points/this.maxPoints*100)}%`;return p`
      <div class="match">
        <small>${e.match}. Spieltag</small>
        <div
          class="match-bar"
          style=${h({background:`linear-gradient(to top, ${i}, ${i}, ${s}, #f5f7f6, ${s}, #f5f7f6);`})}
        >
          ${Array.from({length:e.assists}).map(()=>p`<div>${this.assistSvg}</div>`)}
          ${Array.from({length:e.goals}).map(()=>p`<div>${this.goalSvg}</div>`)}
        </div>
        <small>${e.points}</small>
        <div class="match-team-logos">
          <img class="home-team-logo" src="${f[`team_${e.homeTeamId}`]}" />
          <img class="away-team-logo" src="${f[`team_${e.awayTeamId}`]}" />
        </div>
        ${this.matchResultBadgeSvg(e.homeTeamGoals,e.awayTeamGoals,e.match,t)}
        <small>${Math.round(e.playtimeSeconds/60)}'</small>
      </div>
    `}matchResultBadgeSvg(e,t,i,s){const a=e===-1?"-":String(e??0),r=t===-1?"-":String(t??0);return p`
      <svg class="score-badge">
        ${g`
      <rect x="0" y="0" width="50" height="16" fill="%23#f4f6f5" />
          <text fill="black" font-size="8pt" text-anchor="middle" x="18" y="12" dy="1">${a} : ${r}</text>
  `}
      </svg>
    `}get assistSvg(){return p` <svg width="14" height="14" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ${g`
      <path d="M10.025 11.9412H0.190917C0.00612408 11.9412 -0.0700793 12.1774 0.0785173 12.286L8.03415 18.066C8.13893 18.1422 8.28753 18.0946 8.32753 17.9708L10.2059 12.1907C10.246 12.0669 10.1545 11.9412 10.025 11.9412Z" fill="white"/>
      <path d="M33.0632 11.9411H24.6218C24.4199 11.9411 24.2255 12.0039 24.0617 12.1221L16.9957 17.2563C16.8471 17.3648 16.6452 17.2182 16.7024 17.0429L19.4076 8.71957C19.4705 8.52906 19.4705 8.32141 19.4076 8.1309L16.809 0.131451C16.7519 -0.0438169 16.5042 -0.0438169 16.4471 0.131451L6.28727 31.3939C6.23011 31.5692 6.43205 31.7158 6.58065 31.6073L16.0661 24.7147C16.3995 24.4727 16.8529 24.4727 17.1863 24.7147L26.6717 31.6073C26.8203 31.7158 27.0222 31.5692 26.9651 31.3939L23.3416 20.2434C23.2139 19.851 23.353 19.4204 23.6883 19.1785L33.1737 12.2859C33.3242 12.1773 33.248 11.9411 33.0632 11.9411Z" fill="white"/>
    `}
    </svg>`}get goalSvg(){return p`
      <svg height="12" width="12" viewBox="0 0 1000 1000">
        ${g`
  <g><path fill="white" d="M343,456.3l157-113.8l157,113.8L597.3,640H403.2L343,456.3z M500,10c66.4,0,129.8,12.9,190.3,38.8S803,109.5,846.7,153.3c43.8,43.7,78.6,95.9,104.5,156.4c25.9,60.5,38.8,124,38.8,190.3c0,66.4-12.9,129.8-38.8,190.3c-25.9,60.5-60.7,112.7-104.5,156.4s-95.9,78.6-156.4,104.5C629.8,977.1,566.4,990,500,990c-66.4,0-129.8-12.9-190.3-38.8c-60.5-25.9-112.7-60.7-156.4-104.5S74.7,750.8,48.8,690.3C22.9,629.8,10,566.4,10,500c0-66.4,12.9-129.8,38.8-190.3S109.5,197,153.3,153.3c43.8-43.8,95.9-78.6,156.4-104.5C370.2,22.9,433.6,10,500,10z M838.5,748.3c54.3-74,81.5-156.8,81.5-248.3v-1.6L864.2,547L733,424.5l34.5-176.6l73.3,6.6C786,179.3,715.1,127.9,628,100.2l29,67.8l-157,87l-157-87l29-67.8c-87.1,27.7-158,79.1-212.7,154.2l73.8-6.6L267,424.5L135.8,547L80,498.4v1.6c0,91.5,27.2,174.3,81.5,248.3l16.4-72.2L356.2,698l76,163l-63.4,37.7C411.4,912.9,455.2,920,500,920c44.8,0,88.6-7.1,131.3-21.3l-63.4-37.7l76-163l178.3-21.9L838.5,748.3z"/></g>
    `}
      </svg>
    `}};m.styles=b`
    .root {
      width: 100%;
      display: flex;
      flex-wrap: no-wrap;
      overflow-x: scroll;
      scroll-snap-align: end;
    }

    .season {
      display: flex;
    }

    .season-summary {
      display: flex;
      flex-direction: column;
    }

    .match {
      display: flex;
      flex-direction: column;
      align-items: center;
      margin-right: 10px;
      min-width: 48px;
    }
    .match > small {
      display: inline-flex;
      justify-content: center;
    }

    .match-bar {
      height: 200px;
      background-color: #f5f7f6;
      width: 24px;
      display: flex;
      flex-direction: column-reverse;
      align-items: center;
    }

    .home-team-logo,
    .away-team-logo {
      /* width: 16px; */
      width: 40%;
    }

    .match-team-logos {
      display: flex;
      /* flex */
      justify-content: space-evenly;
    }

    svg.score-badge {
      transform: skew(-10deg);
      width: 80%;
      height: 25px;
    }

    svg.score-badge rect {
      fill: #f4f6f5;
    }
  `;w([c({type:Object})],m.prototype,"points",2);w([c({type:Array})],m.prototype,"upcomingMatches",2);m=w([v(X)],m);var V=Object.defineProperty,q=Object.getOwnPropertyDescriptor,A=(e,t,i,s)=>{for(var a=s>1?void 0:s?q(t,i):t,r=e.length-1,l;r>=0;r--)(l=e[r])&&(a=(s?l(t,i,a):l(a))||a);return s&&a&&V(t,i,a),a};const K="bkb-player";let o=class extends y{async willUpdate(e){if(!this.playerPoints){const{playerInfo:i,playerPoints:s,playerStats:a}=JSON.parse(this.serverJsonData);this.playerInfo=i,this.playerPoints=s,this.playerStats=a}}get upperHalfStyles(){return{"background-color":`var(--team-primary-color-${this.teamId})`}}get colorFadeStyles(){return{background:`linear-gradient(to bottom, transparent, var(--team-primary-color-${this.teamId}));`}}render(){return p`
      <div class="upper-half" style=${h(this.upperHalfStyles)}>
        <img class="player-image" src=${this.playerInfo?.profileBig} alt="picture of ${this.playerName}" />
        <div class="player-color-fade" style=${h(this.colorFadeStyles)}></div>
        <div class="player-summary">
          <div class="player-name">${this.playerName}</div>

          <bkb-player-badges
            .number=${this.playerInfo.number}
            .position=${this.playerInfo.position}
          ></bkb-player-badges>
        </div>
      </div>
      <bkb-player-points
        .points=${this.playerPoints}
        .upcomingMatches=${this.playerStats.upcomingMatches}
      ></bkb-player-points>
    `}};o.styles=b`
    :root,
    * {
      --team-primary-color-15: #28a144;
      --team-primary-color-11: #51a600;
      --team-primary-color-7: #fe0000;
      --team-primary-color-40: #d3011c;
      --team-primary-color-43: #001f46;
      --team-primary-color-3: #fde101;
      --team-primary-color-20: #001f46;
      --team-primary-color-24: #0a5ca5;
      --team-primary-color-13: #ce1719;
      --team-primary-color-14: #1c62b7;
      --team-primary-color-5: #d11b1a;
      --team-primary-color-2: #dc052e;
      --team-primary-color-22: #0090d7;
      --team-primary-color-19: #fde101;
      --team-primary-color-28: #e20612;
      --team-primary-color-9: #e32219;
      --team-primary-color-18: #e30713;
      --team-primary-color-4: #e10010;
    }
    .upper-half {
      width: 100%;
      display: grid;
      grid-template-columns: auto;
      grid-template-rows: auto;
      grid-template-areas: 'main';
    }

    img.player-image {
      grid-area: main;
      width: 100%;
      z-index: 0;
    }

    .player-color-fade {
      grid-area: main;
      width: 100%;
      height: 60%;
      align-self: end;
      z-index: 1;
    }

    .player-summary {
      grid-area: main;
      z-index: 2;
      display: flex;
      flex-direction: column-reverse;
      align-items: center;
    }
  `;A([c({type:String,attribute:"player-name"})],o.prototype,"playerName",2);A([c({type:String,attribute:"player-id"})],o.prototype,"playerId",2);A([c({type:String,attribute:"server-json-data"})],o.prototype,"serverJsonData",2);A([c({type:String,attribute:"team-id"})],o.prototype,"teamId",2);A([Z()],o.prototype,"playerInfo",2);A([Z()],o.prototype,"playerPoints",2);A([Z()],o.prototype,"playerStats",2);o=A([v(K)],o);export{o as PlayerPage,K as tagName};
