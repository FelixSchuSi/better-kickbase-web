import{r as g,e as n,s as b,$ as l,y as p,n as v,i as w,a as y}from"../client-shim.48ed8262.js";import{g as u,p as x}from"./base.service.069015bb.js";var k=Object.defineProperty,R=Object.getOwnPropertyDescriptor,A=(e,t,s,i)=>{for(var a=i>1?void 0:i?R(t,s):t,o=e.length-1,r;o>=0;o--)(r=e[o])&&(a=(i?r(t,s,a):r(a))||a);return i&&a&&k(t,s,a),a};const M="bkb-player-badges";let m=class extends b{constructor(){super(...arguments);this.inverted=!1}render(){return l`
      <div class="badges-container">
        ${this.singleBadge(String(this.number))}
        ${this.singleBadge(u(this.position))}
        ${this.singleBadge(this.status)}
      </div>
    `}singleBadge(e){return e===void 0?"":this.inverted?l`
      ${p`
      <defs>
        <mask id="mask-${e}" x="0" y="0" >
          <rect x="0" y="0" width="32" height="16" fill="#fff" />
          <text text-anchor="middle" x="16" y="12" dy="1">${e}</text>
        </mask>
      </defs>
      <rect x="0" y="0" width="32" height="16" mask="url(#mask-${e})" fill-opacity="1" />
      `}
    `:l`
        ${p`
        <rect x="0" y="0" width="32" height="16" fill="white" />
        <text text-anchor="middle" x="16" y="12" dy="1">${e}</text>
    `}
      `}};m.styles=g`
    .badges-container {
      display: flex;
    }

    svg {
      transform: skew(-10deg);
      height: 16px;
      width: 32px;
    }

    svg:not(:last-child) {
      margin-right: 4px;
    }

    svg text {
      font-weight: 700;
      font-size: smaller;
    }
  `;A([n({type:Number})],m.prototype,"number",2);A([n({type:String})],m.prototype,"position",2);A([n({type:String})],m.prototype,"status",2);A([n({type:Boolean})],m.prototype,"inverted",2);m=A([v(M)],m);var U="/better-kickbase-web/assets/2.b9770161.png",I="/better-kickbase-web/assets/3.c4ecc237.png",Y="/better-kickbase-web/assets/4.18b4e12c.png",P="data:image/png;base64,",E="/better-kickbase-web/assets/7.e4732b84.png",S="/better-kickbase-web/assets/9.0dfad2eb.png",B="/better-kickbase-web/assets/11.0b614101.png",H="/better-kickbase-web/assets/13.9d841356.png",N="/better-kickbase-web/assets/14.63efdbd8.png",O="data:image/png;base64,",G="/better-kickbase-web/assets/18.3780ff0a.png",Q="/better-kickbase-web/assets/19.0896da6a.png",T="/better-kickbase-web/assets/20.ea3c24b0.png",j="data:image/png;base64,",W="data:image/png;base64,",F="/better-kickbase-web/assets/28.a4cea9d1.png",L="data:image/png;base64,",J="/better-kickbase-web/assets/43.a88206a1.png";const f={team_2:U,team_3:I,team_4:Y,team_5:P,team_7:E,team_9:S,team_11:B,team_13:H,team_14:N,team_15:O,team_18:G,team_19:Q,team_20:T,team_22:j,team_24:W,team_28:F,team_40:L,team_43:J};var z=Object.defineProperty,D=Object.getOwnPropertyDescriptor,Z=(e,t,s,i)=>{for(var a=i>1?void 0:i?D(t,s):t,o=e.length-1,r;o>=0;o--)(r=e[o])&&(a=(i?r(t,s,a):r(a))||a);return i&&a&&z(t,s,a),a};const C="bkb-player-points-match";let h=class extends b{constructor(){super(...arguments);this.maxPoints=0}render(){const e=this.match.points>=100?"#25c28b":"#ea5f42",t=`${Math.round(this.match.points/this.maxPoints*100)}%`,s={background:`linear-gradient(to top, ${e}, ${e}, ${t}, #f5f7f6, ${t}, #f5f7f6)`};return l`
      <div class="match">
        <div class="match-day">
          <small class="match-value">${this.match.match}.</small>
          <small class="match-label">Spieltag</small>
        </div>
        <div class="match-bar" style=${w(s)}>
          ${Array.from({length:this.match.assists}).map(()=>l`<div>${this.assistSvg}</div>`)}
          ${Array.from({length:this.match.goals}).map(()=>l`<div>${this.goalSvg}</div>`)}
        </div>
        <small class="match-value points">${this.match.points}</small>
        <div class="match-team-logos">
          <img class="home-team-logo" src="${f[`team_${this.match.homeTeamId}`]}" />
          <img class="away-team-logo" src="${f[`team_${this.match.awayTeamId}`]}" />
        </div>
        ${this.matchResultBadgeSvg(this.match.homeTeamGoals,this.match.awayTeamGoals)}
        <small class="match-value">${Math.round(this.match.playtimeSeconds/60)}'</small>
      </div>
    `}matchResultBadgeSvg(e,t){const s=e===-1?"-":String(e??0),i=t===-1?"-":String(t??0);return l`
      <svg class="score-badge">
        ${p`
      <rect x="0" y="0" width="50" height="16" fill="%23#f4f6f5" />
          <text fill="black" font-size="8pt" text-anchor="middle" x="18" y="12" dy="1">${s} : ${i}</text>
  `}
      </svg>
    `}get assistSvg(){return l` <svg width="14" height="14" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ${p`
      <path d="M10.025 11.9412H0.190917C0.00612408 11.9412 -0.0700793 12.1774 0.0785173 12.286L8.03415 18.066C8.13893 18.1422 8.28753 18.0946 8.32753 17.9708L10.2059 12.1907C10.246 12.0669 10.1545 11.9412 10.025 11.9412Z" fill="white"/>
      <path d="M33.0632 11.9411H24.6218C24.4199 11.9411 24.2255 12.0039 24.0617 12.1221L16.9957 17.2563C16.8471 17.3648 16.6452 17.2182 16.7024 17.0429L19.4076 8.71957C19.4705 8.52906 19.4705 8.32141 19.4076 8.1309L16.809 0.131451C16.7519 -0.0438169 16.5042 -0.0438169 16.4471 0.131451L6.28727 31.3939C6.23011 31.5692 6.43205 31.7158 6.58065 31.6073L16.0661 24.7147C16.3995 24.4727 16.8529 24.4727 17.1863 24.7147L26.6717 31.6073C26.8203 31.7158 27.0222 31.5692 26.9651 31.3939L23.3416 20.2434C23.2139 19.851 23.353 19.4204 23.6883 19.1785L33.1737 12.2859C33.3242 12.1773 33.248 11.9411 33.0632 11.9411Z" fill="white"/>
    `}
    </svg>`}get goalSvg(){return l`
      <svg height="12" width="12" viewBox="0 0 1000 1000">
        ${p`
  <g><path fill="white" d="M343,456.3l157-113.8l157,113.8L597.3,640H403.2L343,456.3z M500,10c66.4,0,129.8,12.9,190.3,38.8S803,109.5,846.7,153.3c43.8,43.7,78.6,95.9,104.5,156.4c25.9,60.5,38.8,124,38.8,190.3c0,66.4-12.9,129.8-38.8,190.3c-25.9,60.5-60.7,112.7-104.5,156.4s-95.9,78.6-156.4,104.5C629.8,977.1,566.4,990,500,990c-66.4,0-129.8-12.9-190.3-38.8c-60.5-25.9-112.7-60.7-156.4-104.5S74.7,750.8,48.8,690.3C22.9,629.8,10,566.4,10,500c0-66.4,12.9-129.8,38.8-190.3S109.5,197,153.3,153.3c43.8-43.8,95.9-78.6,156.4-104.5C370.2,22.9,433.6,10,500,10z M838.5,748.3c54.3-74,81.5-156.8,81.5-248.3v-1.6L864.2,547L733,424.5l34.5-176.6l73.3,6.6C786,179.3,715.1,127.9,628,100.2l29,67.8l-157,87l-157-87l29-67.8c-87.1,27.7-158,79.1-212.7,154.2l73.8-6.6L267,424.5L135.8,547L80,498.4v1.6c0,91.5,27.2,174.3,81.5,248.3l16.4-72.2L356.2,698l76,163l-63.4,37.7C411.4,912.9,455.2,920,500,920c44.8,0,88.6-7.1,131.3-21.3l-63.4-37.7l76-163l178.3-21.9L838.5,748.3z"/></g>
    `}
      </svg>
    `}};h.styles=g`
    .match {
      display: flex;
      flex-direction: column;
      align-items: center;
      margin-right: 10px;
      width: 48px;
      padding-top: 8px;
      padding-bottom: 8px;
    }

    .match > *:not(:last-child) {
      margin-bottom: 8px;
    }

    .match > small {
      display: inline-flex;
      justify-content: center;
    }

    .match-day {
      display: flex;
      flex-direction: column;
      align-items: center;
    }

    .match-value {
      opacity: 87%;
      font-weight: 700;
    }
    .match-value.points {
      font-size: medium;
    }

    .match-label {
      opacity: 60%;
      font-size: smaller;
    }

    .match-bar {
      height: 200px;
      background-color: #f5f7f6;
      width: 24px;
      display: flex;
      flex-direction: column-reverse;
      align-items: center;
    }

    .home-team-logo,
    .away-team-logo {
      width: 40%;
    }

    .match-team-logos {
      display: flex;
      justify-content: space-evenly;
    }

    svg.score-badge {
      transform: skew(-10deg);
      width: 80%;
      height: 16px;
    }

    svg.score-badge rect {
      fill: #f4f6f5;
    }

    svg.score-badge text {
      font-size: 12px;
      opacity: 80%;
    }
  `;Z([n({type:Number,attribute:"max-points"})],h.prototype,"maxPoints",2);Z([n({type:Object})],h.prototype,"match",2);h=Z([v(C)],h);var X=Object.defineProperty,V=Object.getOwnPropertyDescriptor,d=(e,t,s,i)=>{for(var a=i>1?void 0:i?V(t,s):t,o=e.length-1,r;o>=0;o--)(r=e[o])&&(a=(i?r(t,s,a):r(a))||a);return i&&a&&X(t,s,a),a};const q="bkb-player-points";let c=class extends b{constructor(){super(...arguments);this.maxPoints=0}willUpdate(e){e.has("points")&&!!this.points?.seasons&&(this.maxPoints=Math.max(...this.points.seasons.flatMap(t=>t.matches).map(t=>t.points)))}render(){return l`
      <div class="root">
        ${this.points.seasons.map(e=>this.seasonTemplate(e))}
        ${this.upcomingMatches.map(e=>l`<bkb-player-points-match
              .match=${e}
              .maxPoints=${this.maxPoints}
            ></bkb-player-points-match>`)}
      </div>
    `}firstUpdated(e){this.firstUpcomingMatch?this.firstUpcomingMatch?.scrollIntoView(!1):this.lastSeasonSummary?.scrollIntoView(!1)}updated(e){e.has("numberOfSeasonsToRender")&&this.firstSeasonSummary?.scrollIntoView({block:"end",inline:"start"})}seasonTemplate(e){const t=e.appearances>0?Math.round(e.points/e.appearances):0;return l`
      <div class="season">
        ${e.matches.map(s=>l`<bkb-player-points-match .match=${s} .maxPoints=${this.maxPoints}></bkb-player-points-match>`)}
        <div class="season-summary">
          <div class="season-summary-details">
            <div class="season-summary-details-value">${e.year}</div>
            <div class="season-summary-details-key">Saison Auswertung</div>
            <div class="season-summary-details-value">${x.format(e.points)}</div>
            <div class="season-summary-details-key">Punkte</div>
            <div class="season-summary-details-value">${t}</div>
            <div class="season-summary-details-key">∅ Punkte</div>
            <div class="season-summary-details-value">${e.appearances}</div>
            <div class="season-summary-details-key">Einsätze</div>
            <div class="season-summary-details-value">${e.startingEleven}</div>
            <div class="season-summary-details-key">Startelf</div>
          </div>
        </div>
      </div>
    `}};c.styles=g`
    .root {
      width: 100%;
      display: flex;
      overflow-x: scroll;
    }

    .root {
      scrollbar-width: thin;
    }

    .root::-webkit-scrollbar {
      width: 5px;
      height: 8px;
      background-color: #f1f1f1;
    }

    .root::-webkit-scrollbar-thumb {
      background: #c1c1c1;
    }

    .season {
      display: flex;
    }

    .season-summary {
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      border-left: 1px solid #d3d7d8;
      border-right: 1px solid #d3d7d8;
      margin-right: 10px;
      padding-left: 1rem;
      padding-right: 1rem;
    }

    .season-summary-year {
      margin-bottom: 0px;
      margin-top: 0px;
      padding-left: 2rem;
      padding-right: 2rem;
    }

    .season-summary > .season-summary-details-key {
      padding-bottom: 0.4rem;
    }

    .season-summary * {
      text-align: center;
    }

    .season-summary-details {
      display: flex;
      flex-direction: column;
    }

    .season-summary-details-value {
      padding-top: 1rem;
      font-weight: 600;
      opacity: 87%;
      font-size: medium;
    }

    .season-summary-details-key {
      font-size: smaller;
      opacity: 60%;
    }

    .load-season {
      font-family: 'Roboto Condensed';
      font-size: medium;
      background-color: transparent;
      border: 0px solid black;
      border-left: 1px solid #d3d7d8;
      border-right: 1px solid #d3d7d8;
      cursor: pointer;
      padding-left: 1rem;
      padding-right: 1rem;
      max-width: 128px;
    }

    .load-season:hover {
      background-color: #efefef;
    }
  `;d([n({type:Object})],c.prototype,"points",2);d([n({type:Array})],c.prototype,"upcomingMatches",2);d([y(".root > bkb-player-points-match")],c.prototype,"firstUpcomingMatch",2);d([y(".root > .season > .season-summary")],c.prototype,"firstSeasonSummary",2);d([y(".season-summary:last-of-type")],c.prototype,"lastSeasonSummary",2);c=d([v(q)],c);export{f as t};
