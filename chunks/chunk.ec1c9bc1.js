import{r as A,e as n,s as b,g as w,n as v,p as k}from"./chunk.4392602e.js";import{$ as r,y as m,b as x}from"./chunk.322bb44e.js";import{e as R,i as M,t as U}from"./chunk.3206916e.js";/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const I=({finisher:e,descriptor:t})=>(i,s)=>{var a;if(s===void 0){const l=(a=i.originalKey)!==null&&a!==void 0?a:i.key,o=t!=null?{kind:"method",placement:"prototype",key:l,descriptor:t(i.key)}:{...i,key:l};return e!=null&&(o.finisher=function(Z){e(Z,l)}),o}{const l=i.constructor;t!==void 0&&Object.defineProperty(i,s,t(s)),e?.(l,s)}};/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function f(e,t){return I({descriptor:i=>{const s={get(){var a,l;return(l=(a=this.renderRoot)===null||a===void 0?void 0:a.querySelector(e))!==null&&l!==void 0?l:null},enumerable:!0,configurable:!0};if(t){const a=typeof i=="symbol"?Symbol():"__"+i;s.get=function(){var l,o;return this[a]===void 0&&(this[a]=(o=(l=this.renderRoot)===null||l===void 0?void 0:l.querySelector(e))!==null&&o!==void 0?o:null),this[a]}}return s}})}var Y=Object.defineProperty,P=Object.getOwnPropertyDescriptor,h=(e,t,i,s)=>{for(var a=s>1?void 0:s?P(t,i):t,l=e.length-1,o;l>=0;l--)(o=e[l])&&(a=(s?o(t,i,a):o(a))||a);return s&&a&&Y(t,i,a),a};const E="bkb-player-badges";let c=class extends b{constructor(){super(...arguments),this.inverted=!1}render(){return r`
      <div class="badges-container">
        ${this.singleBadge(String(this.number))}
        ${this.singleBadge(w(this.position))}
        ${this.singleBadge(this.status)}
      </div>
    `}singleBadge(e){return e===void 0?"":this.inverted?r`
      ${m`
      <defs>
        <mask id="mask-${e}" x="0" y="0" >
          <rect x="0" y="0" width="32" height="16" fill="#fff" />
          <text text-anchor="middle" x="16" y="12" dy="1">${e}</text>
        </mask>
      </defs>
      <rect x="0" y="0" width="32" height="16" mask="url(#mask-${e})" fill-opacity="1" />
      `}
    `:r`
        ${m`
        <rect x="0" y="0" width="32" height="16" fill="white" />
        <text text-anchor="middle" x="16" y="12" dy="1">${e}</text>
    `}
      `}};c.styles=A`
    .badges-container {
      display: flex;
    }

    svg {
      transform: skew(-10deg);
      height: 16px;
      width: 32px;
    }

    svg:not(:last-child) {
      margin-right: 4px;
    }

    svg text {
      font-weight: 700;
      font-size: smaller;
    }
  `;h([n({type:Number})],c.prototype,"number",2);h([n({type:String})],c.prototype,"position",2);h([n({type:String})],c.prototype,"status",2);h([n({type:Boolean})],c.prototype,"inverted",2);c=h([v(E)],c);/**
 * @license
 * Copyright 2018 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const S=R(class extends M{constructor(e){var t;if(super(e),e.type!==U.ATTRIBUTE||e.name!=="style"||((t=e.strings)===null||t===void 0?void 0:t.length)>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(e){return Object.keys(e).reduce((t,i)=>{const s=e[i];return s==null?t:t+`${i=i.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${s};`},"")}update(e,[t]){const{style:i}=e.element;if(this.ct===void 0){this.ct=new Set;for(const s in t)this.ct.add(s);return this.render(t)}this.ct.forEach(s=>{t[s]==null&&(this.ct.delete(s),s.includes("-")?i.removeProperty(s):i[s]="")});for(const s in t){const a=t[s];a!=null&&(this.ct.add(s),s.includes("-")?i.setProperty(s,a):i[s]=a)}return x}});var B="/better-kickbase-web/assets/asset.b9770161.png",O="/better-kickbase-web/assets/asset.c4ecc237.png",H="/better-kickbase-web/assets/asset.18b4e12c.png",N="data:image/png;base64,",G="/better-kickbase-web/assets/asset.e4732b84.png",T="/better-kickbase-web/assets/asset.0dfad2eb.png",Q="/better-kickbase-web/assets/asset.0b614101.png",j="/better-kickbase-web/assets/asset.9d841356.png",W="/better-kickbase-web/assets/asset.63efdbd8.png",F="data:image/png;base64,",L="/better-kickbase-web/assets/asset.3780ff0a.png",J="/better-kickbase-web/assets/asset.0896da6a.png",z="/better-kickbase-web/assets/asset.ea3c24b0.png",D="data:image/png;base64,",C="data:image/png;base64,",X="/better-kickbase-web/assets/asset.a4cea9d1.png",V="data:image/png;base64,",q="/better-kickbase-web/assets/asset.a88206a1.png";const u={team_2:B,team_3:O,team_4:H,team_5:N,team_7:G,team_9:T,team_11:Q,team_13:j,team_14:W,team_15:F,team_18:L,team_19:J,team_20:z,team_22:D,team_24:C,team_28:X,team_40:V,team_43:q};var K=Object.defineProperty,$=Object.getOwnPropertyDescriptor,y=(e,t,i,s)=>{for(var a=s>1?void 0:s?$(t,i):t,l=e.length-1,o;l>=0;l--)(o=e[l])&&(a=(s?o(t,i,a):o(a))||a);return s&&a&&K(t,i,a),a};const _="bkb-player-points-match";let p=class extends b{constructor(){super(...arguments),this.maxPoints=0}render(){const e=this.match.points>=100?"#25c28b":"#ea5f42",t=`${Math.round(this.match.points/this.maxPoints*100)}%`,i={background:`linear-gradient(to top, ${e}, ${e}, ${t}, #f5f7f6, ${t}, #f5f7f6)`};return r`
      <div class="match">
        <div class="match-day">
          <small class="match-value">${this.match.match}.</small>
          <small class="match-label">Spieltag</small>
        </div>
        <div class="match-bar" style=${S(i)}>
          ${Array.from({length:this.match.assists}).map(()=>r`<div>${this.assistSvg}</div>`)}
          ${Array.from({length:this.match.goals}).map(()=>r`<div>${this.goalSvg}</div>`)}
        </div>
        <small class="match-value points">${this.match.points}</small>
        <div class="match-team-logos">
          <img class="home-team-logo" src="${u[`team_${this.match.homeTeamId}`]}" />
          <img class="away-team-logo" src="${u[`team_${this.match.awayTeamId}`]}" />
        </div>
        ${this.matchResultBadgeSvg(this.match.homeTeamGoals,this.match.awayTeamGoals)}
        <small class="match-value">${Math.round(this.match.playtimeSeconds/60)}'</small>
      </div>
    `}matchResultBadgeSvg(e,t){const i=e===-1?"-":String(e??0),s=t===-1?"-":String(t??0);return r`
      <svg class="score-badge">
        ${m`
      <rect x="0" y="0" width="50" height="16" fill="%23#f4f6f5" />
          <text fill="black" font-size="8pt" text-anchor="middle" x="18" y="12" dy="1">${i} : ${s}</text>
  `}
      </svg>
    `}get assistSvg(){return r` <svg width="14" height="14" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ${m`
      <path d="M10.025 11.9412H0.190917C0.00612408 11.9412 -0.0700793 12.1774 0.0785173 12.286L8.03415 18.066C8.13893 18.1422 8.28753 18.0946 8.32753 17.9708L10.2059 12.1907C10.246 12.0669 10.1545 11.9412 10.025 11.9412Z" fill="white"/>
      <path d="M33.0632 11.9411H24.6218C24.4199 11.9411 24.2255 12.0039 24.0617 12.1221L16.9957 17.2563C16.8471 17.3648 16.6452 17.2182 16.7024 17.0429L19.4076 8.71957C19.4705 8.52906 19.4705 8.32141 19.4076 8.1309L16.809 0.131451C16.7519 -0.0438169 16.5042 -0.0438169 16.4471 0.131451L6.28727 31.3939C6.23011 31.5692 6.43205 31.7158 6.58065 31.6073L16.0661 24.7147C16.3995 24.4727 16.8529 24.4727 17.1863 24.7147L26.6717 31.6073C26.8203 31.7158 27.0222 31.5692 26.9651 31.3939L23.3416 20.2434C23.2139 19.851 23.353 19.4204 23.6883 19.1785L33.1737 12.2859C33.3242 12.1773 33.248 11.9411 33.0632 11.9411Z" fill="white"/>
    `}
    </svg>`}get goalSvg(){return r`
      <svg height="12" width="12" viewBox="0 0 1000 1000">
        ${m`
  <g><path fill="white" d="M343,456.3l157-113.8l157,113.8L597.3,640H403.2L343,456.3z M500,10c66.4,0,129.8,12.9,190.3,38.8S803,109.5,846.7,153.3c43.8,43.7,78.6,95.9,104.5,156.4c25.9,60.5,38.8,124,38.8,190.3c0,66.4-12.9,129.8-38.8,190.3c-25.9,60.5-60.7,112.7-104.5,156.4s-95.9,78.6-156.4,104.5C629.8,977.1,566.4,990,500,990c-66.4,0-129.8-12.9-190.3-38.8c-60.5-25.9-112.7-60.7-156.4-104.5S74.7,750.8,48.8,690.3C22.9,629.8,10,566.4,10,500c0-66.4,12.9-129.8,38.8-190.3S109.5,197,153.3,153.3c43.8-43.8,95.9-78.6,156.4-104.5C370.2,22.9,433.6,10,500,10z M838.5,748.3c54.3-74,81.5-156.8,81.5-248.3v-1.6L864.2,547L733,424.5l34.5-176.6l73.3,6.6C786,179.3,715.1,127.9,628,100.2l29,67.8l-157,87l-157-87l29-67.8c-87.1,27.7-158,79.1-212.7,154.2l73.8-6.6L267,424.5L135.8,547L80,498.4v1.6c0,91.5,27.2,174.3,81.5,248.3l16.4-72.2L356.2,698l76,163l-63.4,37.7C411.4,912.9,455.2,920,500,920c44.8,0,88.6-7.1,131.3-21.3l-63.4-37.7l76-163l178.3-21.9L838.5,748.3z"/></g>
    `}
      </svg>
    `}};p.styles=A`
    .match {
      display: flex;
      flex-direction: column;
      align-items: center;
      margin-right: 10px;
      width: 48px;
      padding-top: 8px;
      padding-bottom: 8px;
    }

    .match > *:not(:last-child) {
      margin-bottom: 8px;
    }

    .match > small {
      display: inline-flex;
      justify-content: center;
    }

    .match-day {
      display: flex;
      flex-direction: column;
      align-items: center;
    }

    .match-value {
      opacity: 87%;
      font-weight: 700;
    }
    .match-value.points {
      font-size: medium;
    }

    .match-label {
      opacity: 60%;
      font-size: smaller;
    }

    .match-bar {
      height: 200px;
      background-color: #f5f7f6;
      width: 24px;
      display: flex;
      flex-direction: column-reverse;
      align-items: center;
    }

    .home-team-logo,
    .away-team-logo {
      width: 40%;
    }

    .match-team-logos {
      display: flex;
      justify-content: space-evenly;
    }

    svg.score-badge {
      transform: skew(-10deg);
      width: 80%;
      height: 16px;
    }

    svg.score-badge rect {
      fill: #f4f6f5;
    }

    svg.score-badge text {
      font-size: 12px;
      opacity: 80%;
    }
  `;y([n({type:Number,attribute:"max-points"})],p.prototype,"maxPoints",2);y([n({type:Object})],p.prototype,"match",2);p=y([v(_)],p);var ee=Object.defineProperty,te=Object.getOwnPropertyDescriptor,g=(e,t,i,s)=>{for(var a=s>1?void 0:s?te(t,i):t,l=e.length-1,o;l>=0;l--)(o=e[l])&&(a=(s?o(t,i,a):o(a))||a);return s&&a&&ee(t,i,a),a};const se="bkb-player-points";let d=class extends b{constructor(){super(...arguments),this.maxPoints=0}willUpdate(e){e.has("points")&&!!this.points?.seasons&&(this.maxPoints=Math.max(...this.points.seasons.flatMap(t=>t.matches).map(t=>t.points)))}render(){return r`
      <div class="root">
        ${this.points.seasons.map(e=>this.seasonTemplate(e))}
        ${this.upcomingMatches.map(e=>r`<bkb-player-points-match
              .match=${e}
              .maxPoints=${this.maxPoints}
            ></bkb-player-points-match>`)}
      </div>
    `}firstUpdated(e){this.firstUpcomingMatch?this.firstUpcomingMatch?.scrollIntoView(!1):this.lastSeasonSummary?.scrollIntoView(!1)}seasonTemplate(e){const t=e.appearances>0?Math.round(e.points/e.appearances):0;return r`
      <div class="season">
        ${e.matches.map(i=>r`<bkb-player-points-match .match=${i} .maxPoints=${this.maxPoints}></bkb-player-points-match>`)}
        <div class="season-summary">
          <div class="season-summary-details">
            <div class="season-summary-details-value">${e.year}</div>
            <div class="season-summary-details-key">Saison Auswertung</div>
            <div class="season-summary-details-value">${k.format(e.points)}</div>
            <div class="season-summary-details-key">Punkte</div>
            <div class="season-summary-details-value">${t}</div>
            <div class="season-summary-details-key">∅ Punkte</div>
            <div class="season-summary-details-value">${e.appearances}</div>
            <div class="season-summary-details-key">Einsätze</div>
            <div class="season-summary-details-value">${e.startingEleven}</div>
            <div class="season-summary-details-key">Startelf</div>
          </div>
        </div>
      </div>
    `}};d.styles=A`
    .root {
      width: 100%;
      display: flex;
      overflow-x: scroll;
    }

    .root {
      scrollbar-width: thin;
    }

    .root::-webkit-scrollbar {
      width: 5px;
      height: 8px;
      background-color: #f1f1f1;
    }

    .root::-webkit-scrollbar-thumb {
      background: #c1c1c1;
    }

    .season {
      display: flex;
    }

    .season-summary {
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      border-left: 1px solid #d3d7d8;
      border-right: 1px solid #d3d7d8;
      margin-right: 10px;
      padding-left: 1rem;
      padding-right: 1rem;
    }

    .season-summary-year {
      margin-bottom: 0px;
      margin-top: 0px;
      padding-left: 2rem;
      padding-right: 2rem;
    }

    .season-summary > .season-summary-details-key {
      padding-bottom: 0.4rem;
    }

    .season-summary * {
      text-align: center;
    }

    .season-summary-details {
      display: flex;
      flex-direction: column;
    }

    .season-summary-details-value {
      padding-top: 1rem;
      font-weight: 600;
      opacity: 87%;
      font-size: medium;
    }

    .season-summary-details-key {
      font-size: smaller;
      opacity: 60%;
    }

    .load-season {
      font-family: 'Roboto Condensed';
      font-size: medium;
      background-color: transparent;
      border: 0px solid black;
      border-left: 1px solid #d3d7d8;
      border-right: 1px solid #d3d7d8;
      cursor: pointer;
      padding-left: 1rem;
      padding-right: 1rem;
      max-width: 128px;
    }

    .load-season:hover {
      background-color: #efefef;
    }
  `;g([n({type:Object})],d.prototype,"points",2);g([n({type:Array})],d.prototype,"upcomingMatches",2);g([f(".root > bkb-player-points-match")],d.prototype,"firstUpcomingMatch",2);g([f(".season:last-of-type > .season-summary")],d.prototype,"lastSeasonSummary",2);d=g([v(se)],d);export{S as i,u as t};
